...
# Normalize data matrix by centering and scaling...
dmats_mean = np.mean(dmats, axis = 0, keepdims = True)
dmats_std  = np.std (dmats, axis = 0, keepdims = True)

# Find zero std
idx_nonzero_std = ~(dmats_std.reshape(-1) == 0.0)

# Normalize
dmats[:, idx_nonzero_std] = (dmats[:, idx_nonzero_std] - dmats_mean[:, idx_nonzero_std]) / dmats_std[:, idx_nonzero_std]
...
np.save("pca.dmats.seq.npy" , dmats)
np.save("pca.u.seq.npy" , u)
np.save("pca.s.seq.npy" , s)
np.save("pca.vh.seq.npy", vh)
np.save("pca.len_seq.npy", len_seq)
