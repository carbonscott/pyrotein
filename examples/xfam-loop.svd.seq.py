...
# Create mask...
for _, (b, e) in labels.items(): mask_loop[b:e+1, :] = 1.0
...
np.save(f"{job_name}.u.seq.npy" , u)
np.save(f"{job_name}.s.seq.npy" , s)
np.save(f"{job_name}.vh.seq.npy", vh)
np.save(f"{job_name}.len_seq.npy", len_seq)
np.save(f"{job_name}.len_res.npy", len_res)
